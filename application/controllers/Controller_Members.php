...
class Controller_Members extends Admin_Controller 
{
	...
	public function index()
	{
		...
		foreach ($user_data as $k => $v) {
			...
			// Find warehouse assigned to this user
			$warehouse = null;
			$warehouses = $this->model_stores->getStoresData();
			foreach ($warehouses as $store) {
				if (isset($store['assigned_user_id']) && $store['assigned_user_id'] == $v['id']) {
					$warehouse = $store;
					break;
				}
			}
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('warehouse_id', 'Warehouse', 'trim|integer');
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if($create == true) {
				// Assign warehouse if selected
				$warehouse_id = $this->input->post('warehouse_id');
				if ($warehouse_id) {
					$this->load->model('model_stores');
					$this->model_stores->update(['assigned_user_id' => $create], $warehouse_id);
				}
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if(empty($this->input->post('password')) && empty($this->input->post('cpassword'))) {
					...
					if($update == true) {
						...
						$warehouse_id = $this->input->post('warehouse_id');
						if ($warehouse_id) {
							$this->model_stores->update(['assigned_user_id' => $id], $warehouse_id);
						}
