...
class Controller_Warehouse extends Admin_Controller 
{
	...
	public function fetchStoresData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if (!empty($value['assigned_user_id'])) {
				$user = $this->model_users->getUserData($value['assigned_user_id']);
				$assigned_user = $user ? htmlspecialchars($user['username'] ?? $user['email']) : '';
			}
			$result['data'][$key] = array(
				$value['name'],
				$assigned_user,
				(int)$value['total_stock'],
				'UGX ' . number_format($value['total_value']),
				$status,
				$buttons
			);
		}
    ...
    */
	public function create()
	{
		...
		$this->form_validation->set_rules('assigned_user_id', 'Assigned User', 'trim|integer');

		$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

		if ($this->form_validation->run() == TRUE) {
			// Check if user is already assigned to another warehouse
			$assigned_user = $this->input->post('assigned_user_id');
			if($assigned_user) {
				$existing = $this->model_stores->getStoreByUserId($assigned_user);
				if($existing) {
			...
			$data = array(
				...
				'assigned_user_id' => $assigned_user ? $assigned_user : null,
			);
    ...
    */
	public function update($id)
	{
		...
		if($id) {
			...
			$this->form_validation->set_rules('edit_assigned_user_id', 'Assigned User', 'trim|integer');

			$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

			if ($this->form_validation->run() == TRUE) {
				// Check if user is already assigned to another warehouse
				$assigned_user = $this->input->post('edit_assigned_user_id');
				if($assigned_user) {
					$existing = $this->model_stores->getStoreByUserId($assigned_user, $id);
					if($existing) {
						$response['success'] = false;
						$response['messages'] = 'This member is already assigned to warehouse "' . $existing['name'] . '". A member can only be assigned to one warehouse.';
						echo json_encode($response);
